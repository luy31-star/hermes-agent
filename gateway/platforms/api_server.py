...
def _build_desktop_system_prompt(request: "web.Request") -> Optional[str]:
    ...
    return (
        ...
        "推荐流程是：先用 knowledge_search 或 wiki_lookup 查找候选，再用 knowledge_read 读取 1 到 3 份最相关条目，必要时再继续扩展。"
        "如果你已经拿到了明确的条目 id，也可以直接调用 knowledge_read。"
        "如果用户只是说“根据知识库”“按之前笔记”“参考 wiki”，这就足以触发知识工具检索。"
        "如果用户消息里出现 `【hermes_desktop_knowledge_refs】` 代码块或类似结构化引用清单，"
        "那主要是知识条目引用和局部摘录，不保证是完整正文；请把其中的 id、标题、来源当作工具读取线索。"
